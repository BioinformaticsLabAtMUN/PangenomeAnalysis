...
def analyze_pangenome(matrix_file, labels_file, output_dir, num_iter=100):
    ...
    # Generate combined plot for both pan-genome and core-genome
    num_genomes = matrix.shape[1]
    sample_pan_curve = df_pan_core.iloc[0, :num_genomes].values  # First iteration, pan data
    sample_core_curve = df_pan_core.iloc[0, num_genomes:].values  # First iteration, core data
    genomes = np.arange(1, num_genomes + 1)
    
    # Fit Heaps law to pan-genome (growth model)
    p0_pan = [0.5, float(min(sample_pan_curve))]
    try:
        popt_pan, _ = scipy.optimize.curve_fit(
            lambda x, alpha, kappa: kappa * x**alpha, 
            genomes, sample_pan_curve, p0=p0_pan
        )
        fitted_pan_curve = popt_pan[1] * genomes**popt_pan[0]
        pan_fit_success = True
    except Exception as e:
        print(f"Warning: Pan-genome curve fit failed: {str(e)}")
        fitted_pan_curve = sample_pan_curve
        popt_pan = [np.nan, np.nan]
        pan_fit_success = False
    
    # Fit exponential decay to core-genome (decay model)
    def core_decay_model(x, kappa_core, decay_lambda, asymptote):
        return kappa_core * np.exp(-decay_lambda * (x - 1)) + asymptote
    
    p0_core = [max(sample_core_curve), 0.1, min(sample_core_curve)]
    try:
        popt_core, _ = scipy.optimize.curve_fit(
            core_decay_model, 
            genomes, sample_core_curve, 
            p0=p0_core,
            bounds=([0, 0, 0], [np.inf, np.inf, max(sample_core_curve)])
        )
        fitted_core_curve = core_decay_model(genomes, *popt_core)
        core_fit_success = True
    except Exception as e:
        print(f"Warning: Core-genome curve fit failed: {str(e)}")
        fitted_core_curve = sample_core_curve
        popt_core = [np.nan, np.nan, np.nan]
        core_fit_success = False
    
    # Create the combined plot
    plt.figure(figsize=(12, 8))
    
    # Plot observed data
    plt.plot(genomes, sample_pan_curve, 'o', color='steelblue', 
             label='Observed pan-genome', markersize=5, alpha=0.7)
    plt.plot(genomes, sample_core_curve, 's', color='darkorange', 
             label='Observed core-genome', markersize=5, alpha=0.7)
    
    # Plot fitted curves
    if pan_fit_success:
        plt.plot(genomes, fitted_pan_curve, '-', color='blue', linewidth=2.5,
                 label=f'Pan-genome fit (Heaps): α={popt_pan[0]:.3f}, κ={popt_pan[1]:.0f}')
    
    if core_fit_success:
        plt.plot(genomes, fitted_core_curve, '-', color='red', linewidth=2.5,
                 label=f'Core-genome fit (Decay): κ={popt_core[0]:.0f}, λ={popt_core[1]:.3f}')
    
    plt.xlabel("Number of genomes", fontsize=13)
    plt.ylabel("Gene count", fontsize=13)
    plt.title("Pan-genome Growth and Core-genome Decay", fontsize=15, fontweight='bold')
    ...
    plt.close()
    print(f"Saved combined pan-core plot to {plot_path}")
